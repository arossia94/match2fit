...
matcher::usage = "matcher[directory,model,looporder,OptionalArguments] takes the file model.fr in directory and related files and performs the matching of SM+model to the SMEFT at looporder level. The only optional argument is QGRAFpath.";
...
$loopLevelSMRGE=0;
SetAttributes[$integLevelSMRGE,{Protected}];
SetAttributes[$loopLevelSMRGE,{Protected}];
...
runSMCouplings[initAssoc_Association, tStart_?NumericQ, tEnd_] :=
  Module[{loopOrder, filteredEqs, inputDispatch, sol},
    ...
    Association[# -> (# @@ {Log10[10^3 Symbol[SymbolName[\[Mu]]]]} /. sol) & /@ Keys[initAssoc]]
  ]
smCoupsAtMatchingMu:=
With[{sol = runSMCouplings[
            <|g ->Subscript[g2, SM],  (* example: derive from mSM inputs *)
              ...
              Ge[2,1]->0,Ge[2,3]->0,
              Ge[3,1]->0,Ge[3,2]->0,
              \[Theta]->0,\[Theta]p->0,\[Theta]s->0|>,
            Log10[mSM[Z]*10^3],          (* tStart \[TildeTilde] 1.96 *)
            Log10[10^3*500]]  (* tEnd   = Log10[500 TeV / GeV], as an upper bound on the matching scale *)
          },
      {Symbol[SymbolName[g1]] -> sol[gp],
       ...
       Symbol[SymbolName[lam]] -> sol[\[Lambda]]/2,
       Symbol[SymbolName[muH]] -> I*Sqrt[sol[m2]]}
    ]
...
matcherMMEFT[directory_,model_,looplevel_,qgrafpath_]:=Block[{listInputFiles,listResultFiles,listProblems,streamScriptMatch,streamScript1,streamScript2},
...
Options[matcher]={"QGRAFPath"->""};
matcher[directory_,model_,looplevel_,OptionsPattern[]]:=Block[{aux},
(*/// Perform Matching with MatchmakerEFT. ///*)
